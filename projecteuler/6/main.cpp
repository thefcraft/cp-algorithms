...
ull squareOfSumOfFirstNaturalNumbers(int n){
    // ap nth term = [ a + (n-1).d ]
    // if n is even
    // a + an = C
    // a+1 + an-1 = C
    // ...
    // sum is n/2*C where C is a + an i.e., [2.a + (n-1).d]
    // else n in odd
    // a + an = C
    // a+1 + an-1 = C
    // ...
    // one element is remained there which is [a + an]/2
    // sum is (n-1)/2*C+C/2 = n/2*C where C is a + an i.e., [2.a + (n-1).d]
    // sum of terms in ap = n/2[ a + a + (n-1).d ]
    n = (n*(n+1))/2;
    return n*n;
}
ull sumOfSquaresOfFirstNaturalNumbers(int n){
    // easy proof using a^3 - b^3 = (a-b)[a^2 + a.b + b^2]
    // then calculate ( n )^3 - (n-1)^3 = ...
    // then calculate (n-1)^3 - (n-2)^3 = ...
    // then calculate (n-2)^3 - (n-3)^3 = ...
    //                ...            
    // then calculate ( 2 )^3 - ( 1 )^3 = ...
    // then calculate ( 1 )^3 - ( 0 )^3 = ...
    // at last take sum of these all together
    // and after rearrangement we can get this formula
...
void solver(){
    const int n = 100;
    cout<<squareOfSumOfFirstNaturalNumbers(n) - sumOfSquaresOfFirstNaturalNumbers(n)<<nl;
}
