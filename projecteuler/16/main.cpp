...
string square(string a){
    ...
    for (int i = 0; i < a.length(); i++) result += toChar(digits[i]);
    delete[] digits;
    return result;
}
...
void solver(){
    string num = "2";
    for (int i = 1; i < 1000; i++) num = square(num);
    int answer = 0;
    for (int i = 0; i < num.length(); i++) answer += toInt(num[i]);
    cout<<answer<<nl;
}
