...
bool isAbundant(int n){
    return factorsSum(n) > n;
}
// TODO: JUST CODE HERE
void solver(){
    vector<int> abundantNumbers;
    for (int i = 1; i <= 28123; i++) if (isAbundant(i)) abundantNumbers.push_back(i);
    int buffer[28123+1] = {0};
    for (int i : abundantNumbers) for (int j : abundantNumbers) if (i+j <= 28123) buffer[i+j]++;
    ull result = 0;
    for (int i = 1; i < 28123+1; i++) if (buffer[i] == 0) result += i;
    cout<<result<<nl;
